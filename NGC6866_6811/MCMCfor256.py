...
mcmctxt = "/data/labs/douglste-laf-lab/wichmand/stardata/mcmc_for_less256.txt"

cat_6811 = QTable.read("/data2/labs/douglste-laf-lab/mathewea/rcat_ngc6811_v0.fits")
cat_6866 = QTable.read("/data2/labs/douglste-laf-lab/mathewea/rcat_ngc6866_v0.fits")
...
def run_mcmc(star_id):
    ...
    joker_file = f"{workpath}/{star_id}/rejection_samples_200.0M_{star_id}_new.hdf5"

    star_dir = f"{workpath}/{star_id}"

    mcmc_file = (
        f"{star_dir}/rejection_samples_MCMC_200.0M_{star_id}_new.hdf5"
    )

    if os.path.exists(mcmc_file):
        print(f"{star_id}: MCMC already exists, skipping")
        return
